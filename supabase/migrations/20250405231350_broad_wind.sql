-- Helper function to generate random age with weighted distribution
...
-- Helper function to generate home location with weighted distribution
...
DECLARE
  european_cities text[] := ARRAY[
    'Amsterdam', 'Berlin', 'Barcelona', 'Paris', 'Rome', 'Prague',
    'Budapest', 'Vienna', 'Copenhagen', 'Madrid', 'Lisbon', 'Athens'
  ];
  asian_cities text[] := ARRAY[
    'Bangkok', 'Hanoi', 'Ho Chi Minh City', 'Singapore', 'Kuala Lumpur',
    'Bali', 'Tokyo', 'Seoul', 'Taipei', 'Hong Kong'
  ];
  ...
  i integer;
BEGIN
  -- Choose city list based on main destination
  IF main_destination = ANY(european_cities) THEN
    available_cities := european_cities;
  ELSE
    available_cities := asian_cities;
  END IF;
  
  -- Remove main destination from available cities
  available_cities := array_remove(available_cities, main_destination);
...
SELECT 
  ...
  generate_random_destinations(
    (ARRAY['Amsterdam', 'Berlin', 'Barcelona', 'Bangkok', 'Bali', 'Tokyo'])[floor(random() * 6 + 1)],
